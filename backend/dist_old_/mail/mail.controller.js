    ...
    async sendEmail(body) {
        try {
            if (!body.settings || !body.email) {
                throw new common_1.HttpException('Missing settings or email data', common_1.HttpStatus.BAD_REQUEST);
            }
            return await this.mailService.sendMail(body.settings, body.email);
        }
        catch (error) {
            throw new common_1.HttpException(error.message || 'Error sending email', common_1.HttpStatus.INTERNAL_SERVER_ERROR);
        }
    }
